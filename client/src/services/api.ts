...
export const boardApi = {
  ...
  async createBoard(
    data: { name: string; ownerId: string; width?: number; height?: number },
    ...
    const response = await fetch(API_BASE_URL, {
      ...
      headers: {
        ...
        // 携带幂等键：兜底重试复用同一键，服务端去重，避免重复创建白板
        ...(options?.idempotencyKey ? { 'Idempotency-Key': options.idempotencyKey } : {}),
      },
      body: JSON.stringify({ ...data, idempotencyKey: options?.idempotencyKey }),
    });
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
